...
// Define a global or configurable debug mode flag
#define ROS_MODE 1



// Declaration of ROS-related functions
rcl_ret_t ros_init();
...
void initialize_hardware() {
    // GPIO initialization for an LED or another indicator
    gpio_init(6);
    gpio_set_dir(6, GPIO_OUT);
    gpio_put(6, 1);

    // Initialize I2C for communication with sensors like the MPU6050
...
void publish_odo(uint64_t current_time)
{
    ...
    odo_msg.pose.pose.position.z = right_speed_target; // z position
    // set the orientation
    ...
    odo_msg.twist.twist.angular.x = motorStatsA.velocity;
    odo_msg.twist.twist.angular.y = motorStatsB.velocity;
...
// Function to map a value from [-1, 1] to [0, 100]
float map_to_0_100(float value) {
    ...
    // Map the value from [-1, 1] to [0, 100]
...
int controlMotorsPID(float dt)
{
    ...
    if (pwmB > 100.0f)
        pwmB = 100.0f;
    imu_msg.linear_acceleration.y = pwmA;
    imu_msg.linear_acceleration.z = pwmB;
    imu_msg.angular_velocity.x =  map_to_0_100(right_speed_target);
    imu_msg.angular_velocity.y = left_speed_target;
    imu_msg.angular_velocity.z = right_speed_target;
    ...
    //    controlLeftMotor(left_speed_target, map_to_0_100(left_speed_target));
    // controlRightMotor(right_speed_target,0);
    // controlLeftMotor(-1.0f,0);
    // controlRightMotor(1.0f,0);
