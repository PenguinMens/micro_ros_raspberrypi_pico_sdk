...
int main() {
    ...
    uint offset = pio_add_program(pio_0, &quadrature_encoder_program);
    quadrature_encoder_program_init(pio_0, sm, offset, PIN_CD, 0);
    uint offset2 = pio_add_program(pio_1, &quadrature_encoder_program);
    quadrature_encoder_program_init(pio_1, sm, offset, PIN_AB, 0);
    while (1) {
        // note: thanks to two's complement arithmetic delta will always
        // be correct even when new_value wraps around MAXINT / MININT
        new_value = quadrature_encoder_get_count(pio_0, sm);
        ...
        // note: thanks to two's complement arithmetic delta will always
        // be correct even when new_value wraps around MAXINT / MININT
        new_value = quadrature_encoder_get_count(pio_1, sm);
        delta = new_value - old_value;
        old_value = new_value;
